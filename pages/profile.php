...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!hash_equals($_SESSION['csrf'] ?? '', $_POST['csrf'] ?? '')) {
        ...
        if (isset($_FILES['avatar']) && $_FILES['avatar']['error'] === UPLOAD_ERR_OK) {
            ...
            if (!in_array($mime, $allowed)) {
                ...
                if (move_uploaded_file($_FILES['avatar']['tmp_name'], $uploadDir . $newName)) {
                    // Delete old avatar if it was a custom one
                    if ($avatarFile !== 'default.jpg' && file_exists($uploadDir . $avatarFile)) {
        ...
        if (empty($errors)) {
            $st = $db->prepare(
                'UPDATE users SET name=?, department=?, semester=?, university=?, skills=?, bio=?, avatar=? WHERE id=?'
            );
            $st->execute([$name, $department, $semester, $university, $skills, $bio, $avatarFile, $user['id']]);
            $user    = currentUser(true); // bust cache
            $success = true;
...
$depts = []; // kept for compat — cascade now driven by DEPT_COURSES_PHP
    ...
    <div class="row g-4">
      ...
      <div class="col-lg-9">
        <div style="border:1.5px solid var(--ink);background:var(--white);padding:36px;" class="reveal d1">
          ...
          <div class="alert alert-danger cc-alert"><i class="fas fa-triangle-exclamation me-2"></i><?= e($errors['avatar']) ?></div>
          <?php endif; ?>
          ...
          <form method="POST" enctype="multipart/form-data" novalidate>
            ...
            <div class="row g-3">
              ...
              <!-- Avatar upload -->
              <div class="col-12">
                <label class="cc-form-label">Profile Photo <span style="color:#aaa;font-size:.65rem;">(JPG/PNG/WebP, max 2 MB)</span></label>
                <div class="d-flex align-items-center gap-3">
                  <img id="avatarPreviewSmall" src="<?= e(avatarUrl($user)) ?>"
                       style="width:56px;height:56px;object-fit:cover;border:2px solid var(--ink);" alt="">
                  <div>
                    <input type="file" name="avatar" id="avatarInput" accept="image/*"
                           style="font-size:.82rem;color:var(--ink);"
                           onchange="previewAvatar(this)">
                    <div style="font-size:.7rem;color:#aaa;margin-top:4px;font-family:var(--font-mono);">
              ...
              <!-- Step 1: Department group (UI-only) -->
              <div class="col-md-4">
                ...
                <select id="profDeptGroup" class="cc-form-input"
                        onchange="onDeptChange('profDeptGroup','profCourse','profSem','profCourseInfo')">
              ...
              <!-- Step 2: Course / Programme (submitted as 'department') -->
              <div class="col-md-5">
                <label class="cc-form-label">
                  Course / Programme
                  <span style="font-size:.58rem;color:#bbb;font-family:var(--font-mono);font-weight:400;margin-left:4px;">② Then pick course</span>
                </label>
                <select id="profCourse" name="department" class="cc-form-input" disabled
                        onchange="onCourseChange('profCourse','profSem','profCourseInfo')">
                  <option value="">— Select department first —</option>
                </select>
              </div>

              <!-- Step 3: Semester (auto-updated) -->
              <div class="col-md-3">
                <label class="cc-form-label">
                  Semester
                  <span style="font-size:.58rem;color:#bbb;font-family:var(--font-mono);font-weight:400;margin-left:4px;">③ Auto-set</span>
                </label>
                <select name="semester" id="profSem" class="cc-form-input" disabled>
                  <option value="">—</option>
                </select>
                <div id="profCourseInfo" style="font-size:.64rem;color:var(--rust);font-family:var(--font-mono);margin-top:4px;letter-spacing:.02em;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"></div>
              </div>
        ...
        <div style="border:1.5px solid var(--ink);background:var(--white);padding:36px;margin-top:20px;" class="reveal d2">
          ...
          <form method="POST" action="change_password.php" novalidate>
            ...
            <div class="row g-3">
              ...
              <div class="col-md-4">
                <label class="cc-form-label">New Password <span style="font-size:.62rem;color:#aaa;">(min 8)</span></label>
                <input type="password" name="new_password" class="cc-form-input" placeholder="Min 8 characters">
              </div>
              <div class="col-md-4">
...
function onCourseChange(courseId, semId, infoId, preselect) {
  ...
  const savedSem = parseInt(sSel.value, 10) || <?= (int)($user['semester'] ?? 1) ?>;
  const curSem   = Math.min(savedSem, meta.sem);
  sSel.innerHTML  = '';
...
document.addEventListener('DOMContentLoaded', function () {
  const saved = <?= json_encode($user['department'] ?? '') ?>;
  const sem   = <?= (int)($user['semester'] ?? 1) ?>;
  if (!saved) return;
  const meta = COURSE_META[saved];
  if (!meta) return;
  document.getElementById('profDeptGroup').value = meta.group;
  onDeptChange('profDeptGroup', 'profCourse', 'profSem', 'profCourseInfo', saved);
  if (document.getElementById('profSem')) {
    document.getElementById('profSem').value = Math.min(sem, meta.sem);
  }
});
</script>
